---
version: alpha
name: v3-design (Framer-structured, v3 scheme)
description: "Adam Tang's engineering-portfolio design system, documented in getdesign's Framer-inspired STRUCTURE but using v3's own scheme: a light hero/about register (cool off-white paper, dark ink type) that flows into a dark, cinematic projects gallery (near-black canvas, mist type), unified by a single signal-orange accent (#FF5A1F) reserved for CTAs, hover, active toggles, and key metrics. Display type is Clash Display with tight negative tracking; body is General Sans; technical labels/specs are JetBrains Mono. Keep Framer's spacing, radius, component anatomy and responsive strategy as reference — but the colors and fonts here are the source of truth, not Framer's blue/gradient/dark-only guidance."

colors:
  # ── v3 SCHEME (kept). Light hero/about + dark gallery, one signal-orange accent.
  #    Framer's structure below is reference only; colors/fonts are overridden here.
  accent: "#FF5A1F"          # signal orange — the single brand accent
  accent-ink: "#D33E08"      # darker orange for AA text on light
  accent-glow: "#FF7A45"     # orange glow on dark
  accent-blue: "#FF5A1F"     # alias for the template's accent slot → recolored to signal orange
  # Light register — hero, about, featured work, skills
  paper: "#F4F5F6"           # page canvas (light)
  surface: "#FFFFFF"         # cards on light
  ink: "#15171B"             # primary text on light
  ink-700: "#3B3F46"
  ink-muted: "#6A6F77"
  ink-400: "#8A8F97"
  line: "#E3E5E8"            # hairline on light
  # Dark register — projects gallery, project detail, footer
  canvas: "#0C0D10"          # night-900 (dark page)
  surface-1: "#121419"       # night-800 (cards on dark)
  surface-2: "#1A1D23"       # night-700 (featured cards on dark)
  hairline: "#23272E"        # night-600
  hairline-soft: "#1A1D23"
  mist: "#F1F2F4"            # primary text on dark
  mist-dim: "#9CA1A8"
  mist-faint: "#6E737B"
  # CTA fills
  primary: "#15171B"         # light-register primary CTA = ink pill (paper text)
  on-primary: "#F4F5F6"
  primary-dark: "#FFFFFF"    # dark-register primary CTA = white pill (ink text)
  inverse-canvas: "#FFFFFF"
  inverse-ink: "#15171B"
  semantic-success: "#22c55e"
  # Optional accent gradients (Framer-signature spotlight cards) — recolored on-brand, use sparingly
  gradient-orange: "#FF5A1F"
  gradient-magenta: "#FF7A45"
  gradient-violet: "#FF5A1F"
  gradient-coral: "#D33E08"
...
> **v3 scheme override (read first).** This file uses getdesign's Framer analysis as a
> *structural* reference — spacing, radius, component anatomy, responsive strategy. The
> **colors and fonts in the front matter are this project's own and take precedence**: a
> **light** hero/about register (`paper`/`ink`) that flows into a **dark** gallery
> (`canvas`/`mist`), one **signal-orange** accent (`accent` `#FF5A1F`) — not Framer's blue.
> Where the Framer prose below says "dark-only", "blue accent", or "vibrant gradient
> spotlight cards", treat that as Framer's identity, not ours; our accent is orange, our
> gradients (if used at all) are the recolored orange set, and we ship both light and dark.
> Fonts: **Clash Display** (display), **General Sans** (body), **JetBrains Mono** (labels/specs).
