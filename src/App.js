...
function HomePage({ aboutImages }) {
    const { scrollY } = useScroll();
    const imageOffset = useTransform(scrollY, (value) => value * 0.1);
    const contentOffset = useTransform(scrollY, (value) => value * -0.1);
    const location = useLocation();
    const navigate = useNavigate();
    const aboutImageList = useMemo(() => (aboutImages.length > 0 ? aboutImages : ["/images/about.jpg"]), [aboutImages]);
...
function ProjectCard({ project, delay }) {
    ...
    return (_jsx(motion.article, { initial: { opacity: 0, y: 24 }, whileInView: { opacity: 1, y: 0 }, viewport: { once: true, amount: 0.3 }, transition: { duration: 0.6, ease: "easeOut", delay }, className: "group relative h-[320px] overflow-hidden rounded-[3rem] border border-white/10 bg-[#1d2027]/75 shadow-[0_30px_100px_-70px_rgba(0,0,0,0.6)] backdrop-blur-xl", onMouseMove: (event) => {
